...
proc fileLexer*(filename:string): iterator(): Token =
  var stream = llStreamOpen(filename, fmRead)
  if stream == nil: rawMessage(errCannotOpenFile, filename)
  var lex: Lexer
  openLexer(lex, filename, stream)
  return iterator(): Token =
    while true:
      var token: Token
      lex.getTok(token)
      if token.xkind == pxEof:
        break
      else:
        yield token

when isMainModule:
  var lexfile = fileLexer("bwapi/include/bwapi.h")
  for tok in lexfile():
    echo "[",tok.xkind,"] ", tok.s
